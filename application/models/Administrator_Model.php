...
class Administrator_Model extends CI_Model
{
	...
	public function add_permisions($role_id)
	{
		$data = array(
			'admin_type' => $role_id,
			'creat'      => 1,
		...
		$this->db->insert('permisions', $data);
		return $this->db->insert_id();
	}
	...
	public function get_permisions()
	{
		$query = $this->db->get('permisions');
		return $query->result();
	}
	// get single record of user role
	public function get_user_role_name($id)
	{
		$query = $this->db->get_where('admin_type', ['id' => $id]);
		return $query->row();
	}

	// get result set of user role
	...
	public function update_user_permision_data($permision_id, $data)
	{
		// $data = array(
		// 	'name'        => $this->input->post('name'),
		// 	'domain'      => $this->input->post('domain'),
		...
		// 	'status'      => $this->input->post('status')
		// );
		$this->db->where('id', $permision_id);
		return $this->db->update('permisions', $data);
		
	}

	public function get_current_user_permision($id)
	{
		$query = $this->db->get_where('permisions', ['admin_type' => $id]);
		return $query->row();
	}

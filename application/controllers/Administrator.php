...
class Administrator extends CI_Controller
{
	...
	{
		parent::__construct();
		$this->permision = $this->Administrator_Model->get_current_user_permision($this->session->userdata('role'));
	}
	...
	// user role
	public function role($page = 'user_role')
	{
		if (!file_exists(APPPATH . 'views/administrator/' . $page . '.php')) {
			show_404();
		}
		...
		$this->form_validation->set_rules('type', 'User Rle', 'required');
		$this->form_validation->set_rules('name', 'Name', 'required');
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$role_id = $this->Administrator_Model->add_user_role();
			
			$this->Administrator_Model->add_permisions($role_id);
			//Set Message
			$this->session->set_flashdata('success', 'User has been created Successfull.');
			redirect('administrator/users');
		}
	...
	// user permision
	public function permision($page = 'permision')
	{
		if (!file_exists(APPPATH . 'views/administrator/' . $page . '.php')) {
			show_404();
		}
		$data['title'] = ucfirst($page);
		$data['permisions'] = $this->Administrator_Model->get_permisions();
		$this->load->view('administrator/header-script');
		$this->load->view('administrator/header');
		$this->load->view('administrator/header-bottom');
		$this->load->view('administrator/' . $page, $data);
	...
	// update permision
	public function update_permision($id)
	{
		$data = [];
		$this->input->post('creat') ? $data['creat'] = 1 : $data['creat'] = 0;
		$this->input->post('view') ? $data['view'] = 1 : $data['view'] = 0;
		$this->input->post('edit') ? $data['edit'] = 1 : $data['edit'] = 0;
		$this->input->post('remove') ? $data['remove'] = 1 : $data['remove'] = 0;
		// echo '<pre>';
		// // print_r($id);
		// print_r($data);
		// echo '</pre>';
		// exit;
		$this->Administrator_Model->update_user_permision_data($id, $data);
		// echo '<pre>';
		// print_r($this->db->last_query());
		// echo '</pre>';
		// exit;
		//Set Message
		$this->session->set_flashdata('success', 'User Permision has been Updated Successfull.');
		redirect('administrator/users-permision');
	}
